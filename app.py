...
# --- AI Configuration ---
# Use st.secrets for deployment or os.getenv for local testing
try:
...
if uploaded_file and st.button("Generate AI Draft"):
    with st.spinner("Gemini 2.5 Flash is analyzing your document..."):
        try:
            ...
            # AI Instructions for formatting
            prompt = f"""
            ...
            CRITICAL RULE: ONLY use the '|' symbol to separate the Company/Degree and the Date. DO NOT use '|' anywhere else. If there are multiple job titles (e.g. 'Manager | Lead'), combine them with a hyphen (e.g. 'Manager - Lead') so it is read as one single job title line.
            ...
            # --- API CALL WITH RATE LIMIT PROTECTION ---
            if input_data:
                try:
                    ...
                    if "429" in str(ai_err):
                        st.error("⚠️ Server Busy: Too many requests at once. Please wait 15 seconds and try again.")
                    else:
...
if st.session_state.edited_content:
    ...
    if st.button("Apply to Company Template & Download"):
        ...
        if os.path.exists(t_file):
            ...
            st.warning("Template file not found. Generating standard document.")
            doc = docx.Document()
        ...
        for line in st.session_state.edited_content.split('\n'):
            ...
            # Bullet points for specific sections
            if any(bh in current_section for bh in bullet_headers):
                doc.add_paragraph(line.lstrip("*-• "), style='List Bullet').paragraph_format.space_after = Pt(2)
            
            # Table Logic for Company | Date
            elif "|" in line:
                parts = line.split("|")
                p = doc.add_paragraph()
                ...
                # Right-align the date using a tab (assuming template has a right tab stop)
        ...
        doc.save(buf)
        st.download_button(
            label=f"Download {company_choice} Resume",
            data=buf.getvalue(),
