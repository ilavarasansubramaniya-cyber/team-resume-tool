...
# --- GLOBAL SETTINGS ---
UNIFORM_SPACE = Pt(10) 
ENTRY_SPACING = Pt(12) # Space between one experience/education and the other
...
def set_arial_font(doc):
    ...
    font.name = 'Arial'
    font.size = Pt(11)
    # This ensures the font applies to document parts that don't strictly follow 'Normal'
...
if st.session_state.edited_content:
    ...
    if st.button("Download Final Word Doc"):
        ...
        for line in st.session_state.edited_content.split('\n'):
            ...
            # Experience/Education Table with Spacing
            elif "|" in line:
                # Add spacing before the new experience/education entry
                spacer = doc.add_paragraph()
                spacer.paragraph_format.space_before = ENTRY_SPACING
                ...
                run_comp.bold, run_comp.font.name = True, 'Arial' # Company/Univ stays BOLD
            ...
            # Job Title / Degree Logic
            else:
                p_body = doc.add_paragraph()
                if last_line_was_company:
                    run_job = p_body.add_run(line.title())
                    run_job.bold = False # Job Title / Degree is NOT BOLD
        ...
        footer_run = p_foot.add_run(f"\nIf you would like to interview this candidate, please call {contact_number}")
        footer_run.bold, footer_run.font.name = True, 'Arial'
        footer_run.font.color.rgb = RGBColor(0, 51, 153)

        buf = io.BytesIO()
        doc.save(buf)
        st.download_button("Download Arial Resume", buf.getvalue(), f"{company_choice}_Resume_Arial.docx")
